...
<div class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow">
    <h1 class="text-2xl font-bold text-blue-600 mb-6">Sửa mã khuyến mãi: {{ $coupon->code }}</h1>
    ...
    <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="code" value="{{ old('code', $coupon->code) }}" required class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>
        ...
        <div>
            ...
            <input type="number" name="value" value="{{ old('value', $coupon->value) }}" min="0" required class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>

        <div>
            <label class="block text-sm font-semibold mb-1">Đơn tối thiểu</label>
            <input type="number" name="min_order_value" value="{{ old('min_order_value', $coupon->min_order_value) }}" min="0" required class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>

        <div>
            <label class="block text-sm font-semibold mb-1">Giới hạn lượt dùng</label>
            <input type="number" name="usage_limit" value="{{ old('usage_limit', $coupon->usage_limit) }}" min="1" class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>

        <div>
            <label class="block text-sm font-semibold mb-1">Ngày hết hạn</label>
            <input type="date" name="expires_at" value="{{ old('expires_at', $coupon->expires_at ? $coupon->expires_at->format('Y-m-d') : '') }}" class="w-full border border-slate-200 rounded-lg px-3 py-2">
        </div>
